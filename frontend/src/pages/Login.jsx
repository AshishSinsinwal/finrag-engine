...
import { Mail, Lock, ArrowRight, Layers, AlertCircle, Loader2 } from "lucide-react";
      ...
      <div className="w-full max-w-md bg-app-surface border border-app-border rounded-2xl p-8 sm:p-10 shadow-premium relative z-10">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="text-sm text-app-text-secondary mt-1 font-medium">
            Sign in to FinWise Intelligence
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-app-text text-white py-3 rounded-xl font-semibold flex items-center justify-center gap-2 hover:bg-black transition-all disabled:opacity-70 disabled:cursor-not-allowed mt-2"
          >
            {loading ? (
              <>
        ...
        <p className="mt-8 text-center text-sm font-medium text-app-text-secondary border-t border-app-border pt-6">
          ...
          <Link to="/register" className="ml-1.5 text-app-accent hover:text-blue-700 transition-colors">
            Create workspace
          </Link>
        </p>
      </div>
